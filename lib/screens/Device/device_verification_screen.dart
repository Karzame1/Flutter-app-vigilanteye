      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            Observer(
              builder: (_) {
                debugPrint("device..${_store.deviceVerificationStatus}");
            ...
            Observer(builder: (_) {
              if (_store.deviceVerificationStatus ==
                ...
                ///Todo-
                /// change the return button after testing with the lower one
                /// because it is just for can login not asking for log out when same account registered with other device
                /// admin will verify why using other device than admin clear previous device registered
                /// after that agent can login with different device.
                return button(language!.lblOk, onTap: () async {
                  if (getBoolAsync(isDeviceVerifiedPref)) {
                    await sharedHelper.refreshAppSettings();
                    toast(language!.lblWelcomeBack);
